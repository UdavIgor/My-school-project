...
def apply_yolo_object_detection(image_to_process):
    ...
    # Starting a search for objects in an image    for out in outs:
    for out in outs:
        for obj in out:
            scores = obj[5:]
...
if __name__ == '__main__':
    ...
    # Loading YOLO scales from files and setting up the network    net = cv2.dnn.readNetFromDarknet(
    net = cv2.dnn.readNetFromDarknet ("D:\Makarevskiy Igor\Resources\yolov4-tiny.cfg",
                                      "D:\Makarevskiy Igor\Resources\yolov4-tiny.weights")
    ...
    with open("D:\Makarevskiy Igor\Resources\coco.names.txt") as file:
        classes = file.read().split("\n")

    #Video capture
    capture = cv2.VideoCapture("D:\Screen\istockphoto-1287947740-640_adpp_is.mp4")
    while (True):
        success, frame = capture.read()
        if success:
            cv2.imwrite(f"D:\Screen\Screen.jpg", frame)
